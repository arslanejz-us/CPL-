      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-gray-200">
          {FEATURES.map(({ title, desc, Icon }) => (
            <div
              key={title}
              className="flex items-start gap-3 p-4 lg:px-6"
            >
              <span className="text-brand-primary flex-shrink-0">
                <Icon className="w-8 h-8" strokeWidth={1.75} />
              </span>
              <div>
                <h3 className="font-display font-bold text-[13px] uppercase tracking-wide text-brand-secondary">
                  {title}
                </h3>
                <p className="font-montserrat text-[11px] text-gray-500 mt-1 leading-relaxed">
                  {desc}
                </p>
              </div>
            </div>
